...
public class HudSettings {
    ...
    private float hudScale = 1.0f; // нова змінна

    public float getHudScale() { return hudScale; }
    public void setHudScale(float scale) {
        hudScale = Math.max(0.5f, Math.min(3.0f, scale)); // обмеження від 0.5 до 3.0
    }
    ...
    public void load() {
        ...
        try (InputStream in = new FileInputStream(configFile)) {
            ...
            hudScale = Float.parseFloat(props.getProperty("hudScale", "1.0")); // додано
    ...
    public void save() {
        try (OutputStream out = new FileOutputStream(configFile)) {
            ...
            props.setProperty("hudScale", Float.toString(hudScale)); // додано
